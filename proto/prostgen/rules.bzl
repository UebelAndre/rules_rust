...
_PROST_COMPILE_PROC_MACRO_DEPS = [
    "@rules_rust//proto/prostgen/raze/cargo:prost_derive",
]
...
def _prost_generator_impl(ctx):
    ...
    transitive_sources = depset(transitive = [
        ...
        depset(f[ProtoInfo].transitive_sources)
        for f in ctx.attr.deps
    ])
    ...
    rustfmt = ctx.toolchains[str(Label("//rust:toolchain"))].rustfmt
    ctx.actions.run(
        inputs = depset(
...
def _prost_tonic_library(name, deps, grpc, extern = None, **kwargs):
    ...
    Args:
        ...
        deps ([type]): [description]
        grpc ([type]): [description]
        extern ([type], optional): [description]. Defaults to None.
    ...
    dependencies = _PROST_COMPILE_DEPS
    if grpc:
        dependencies = dependencies + _TONIC_COMPILE_DEPS
    _prost_generator(
    ...
    rust_library(
        ...
        proc_macro_deps = _PROST_COMPILE_PROC_MACRO_DEPS,
        edition = "2018",
        **kwargs
    )
