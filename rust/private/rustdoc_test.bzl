...
def _construct_writer_arguments(ctx, toolchain, test_runner, opt_test_params, action, crate_info):
    ...
    writer_args.add(toolchain.process_wrapper.short_path)
...
def _legacy_rust_doc_test_impl(ctx, toolchain, crate_info):
    ...
    tools = action.tools + [toolchain.process_wrapper]
