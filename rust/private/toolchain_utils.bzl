...
def generate_sysroot(ctx, exec_toolchain = None, target_toolchain = None):
    ...
    if not target_toolchain:
        ...
        target_deps = []
        target_transitive_deps = [target_toolchain.rust_stdlib.files]
...
def _toolchain_target_files_impl(ctx):
    toolchain = ctx.toolchains[str(Label("//rust:target_toolchain"))]
...
def _toolchain_cargo_files_impl(ctx):
    ...
    exec_toolchain = ctx.toolchains[str(Label("//rust:exec_toolchain"))]
