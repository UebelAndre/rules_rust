...
def _collect_render_config(*, module):
    ...
    if len(module.tags.render_config) > 1:
        fail("Multiple render configs provided for module `{}`. Only 1 is allowed.".format(
            module.name,
        ))

    config = None
    if module.tags.render_config:
        raw_config = module.tags.render_config[0]
        ...
        config = json.decode(render_config(**config_kwargs))
    else:
        config = json.decode(render_config())
...
def _collect_splicing_config(*, module):
    ...
    """
    if len(module.tags.splicing_config) > 1:
        fail("Multiple render configs provided for module `{}`. Only 1 is allowed.".format(
            module.name,
        ))

    if module.tags.splicing_config:
        raw_config = module.tags.splicing_config[0]
        config_kwargs = {attr: getattr(raw_config, attr) for attr in dir(raw_config)}

        config = json.decode(splicing_config(**config_kwargs))
    else:
        config = json.decode(splicing_config())

    return config

def _generate_hub_and_spokes(*, module_ctx, cargo_bazel, cfg, annotations, rendering_config, splicing_config, cargo_lockfile = None, manifests = {}, packages = {}):
    ...
    Args:
        ...
        rendering_config (dict): The render config to use.
