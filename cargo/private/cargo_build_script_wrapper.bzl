...
def cargo_build_script(
    ...
    # The script is always run in an `exec` configuration so whenever the `cargo_build_script` is given
    # an `exec_compatible_with`, for this target it is translated to `target_compatible_with` to match
    # the `cfg = "exec"` consumption of the target.
