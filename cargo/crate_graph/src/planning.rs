...
mod tests {
    ...
    fn mock_workspace_crate_metadata(metadata_template: &str) -> CrateMetadata {
        ...
        let raze_metadata = fetcher.fetch_metadata(dir.as_ref(), None).unwrap();
    ...
    fn test_plan_build_produces_aliased_dependencies() {
        ...
        let crates_with_aliased_deps: Vec<CrateContext> = planned_build_res
            ...
            .crate_contexts
            .into_iter()
            .filter(|ctx| ctx.default_deps.aliased_dependencies.len() != 0)
            .collect();
    ...
    fn mock_workspace_member_toml_contents(name: &str, dep_version: &str) -> String {
        assert!(
            ...
            "The `mock_workspace_members_metadata` template was generated with these versions, if something else is passed, that file will need to be regenerated",
        );
        indoc::formatdoc! { r#"
            [package]
            name = "{name}"
    ...
    fn mock_workspace_members_metadata() -> CrateMetadata {
        ...
        fetcher.fetch_metadata(crate_dir.as_ref(), None).unwrap()
    }
    ...
    fn test_workspace_members_share_dependency_of_different_versions() {
        ...
        assert!(
            planned_build
                .crate_contexts
                .iter()
                .find(|ctx| ctx.pkg_name == "unicode-xid"
                    && ctx.pkg_version == Version::from((0, 1, 0)))
                .is_some()
        );
        ...
        assert!(
            planned_build
                ...
                .iter()
                .find(|ctx| ctx.pkg_name == "unicode-xid"
                    && ctx.pkg_version == Version::from((0, 2, 1)))
                .is_some()
        );
    }
