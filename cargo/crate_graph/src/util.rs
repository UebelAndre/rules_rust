...
pub fn get_workspace_member_path<T: AsRef<Path>>(
    manifest_path: &T,
    workspace_root: &T,
) -> Option<PathBuf> {
    assert!(manifest_path.as_ref().ends_with("Cargo.toml"));
    // UNWRAP: A manifest path should always be a path to a 'Cargo.toml' file which should always have a parent directory
    diff_paths(manifest_path.as_ref().parent().unwrap(), workspace_root)
}
