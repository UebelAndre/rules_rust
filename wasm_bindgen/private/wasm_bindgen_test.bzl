...
load("//rust/private:rust.bzl", "get_rust_test_flags")

# buildifier: disable=bzl-visibility
load("//rust/private:rustc.bzl", "rustc_compile_action")

# buildifier: disable=bzl-visibility
load("//rust/private:toolchain_utils.bzl", "get_coverage_env")
...
def _rust_wasm_bindgen_test_impl(ctx):
    ...
    env["BROWSER"] = rlocationpath(ctx, wb_toolchain.browser)
    if wb_toolchain.browser_type == "chrome":
        env["CHROMEDRIVER"] = rlocationpath(ctx, wb_toolchain.webdriver)
    elif wb_toolchain.browser_type == "firefix":
        env["GECKODRIVER"] = rlocationpath(ctx, wb_toolchain.webdriver)
    else:
    ...
    for prov in crate_providers:
        if type(prov) == "DefaultInfo":
            env["TEST_WASM_BINARY"] = rlocationpath(ctx, prov.files_to_run.executable)
...
rust_wasm_bindgen_test = rule(
    ...
    attrs = {
        ...
        "_runner": attr.label(
            ...
            cfg = "exec",
            executable = True,
            default = Label("//wasm_bindgen/private:wasm_bindgen_test_runner"),
        ),
    },
    test = True,
)
